...
class AgentExecutor:
    ...
    def run(
        ...
        """
        from config import (
            AGENT_MODEL, AGENT_MAX_ITERATIONS, SYSTEM_PROMPT_AGENT,
            COMPUTER_USE_ENABLED, COMPUTER_USE_BETA,
        )
        ...
        logger.info("[agent] Starting task with %d tools (computer_use=%s), max %d iterations",
                     len(tool_defs), use_computer, max_iterations)
        ...
        for iteration in range(max_iterations):
            ...
            if response.stop_reason != "tool_use":
                ...
                logger.info(
                    ...
                    elapsed, iteration + 1, final_text or "<empty>",
                )
                return final_text
            ...
            for tool_use in tool_uses:
                logger.info("[agent] Tool call: %s(%s)",
                            tool_use.name, _short_json(tool_use.input))
            ...
            if string_results:
                empty_count = sum(
                    1 for r in string_results
                    if any(m in r["content"].lower() for m in empty_markers)
                )
    ...
    def _track_usage(self, response) -> None:
        ...
        try:
            ...
            self._cost_tracker.record("smart", input_tokens, output_tokens)
